...
AEABISPECIFIC[x86]="-Wno-error -Wno-implicit-function-declaration"
AFLAGS[x86]="-Wno-error -Wno-implicit-function-declaration "
...
AEABISPECIFIC[x86_64]="-Wno-error -Wno-implicit-function-declaration"
AFLAGS[x86_64]="-Wno-error -Wno-implicit-function-declaration"
ACOMPILER_TRIP[x86_64]=x"86_64-linux-android"
...
export COMPILER_ROOT=${NDK}/${TARGET}
export SYSROOT=${COMPILER_ROOT}/sysroot
export TOOLCHAIN_PATH=${COMPILER_ROOT}/bin
export PATH=${SYSROOT}/usr/bin:${TOOLCHAIN_PATH}:${PATH}
export TOOLCHAIN_PREFIX=${TOOLCHAIN_PATH}/${COMPILER_TRIP}
...
export PYTHON_CFLAGS="-I${COMPILER_ROOT}/include/python2.7"
export PYTHON_LDFLAGS="-L${COMPILER_ROOT}/lib/python2.7"
 

  CFLAGS="${CFLAGS}  -fPIE -fPIC -march=${MARCH} -DANDROID  -grecord-gcc-switches  -ffunction-sections -funwind-tables -fstack-protector-strong -no-canonical-prefixes -mstackrealign -Wa,--noexecstack -Wformat -Werror=format-security -std=c11  ${EABISPECIFIC} ${AFLAGS[${ARMEABI}]}  $(getLDflags ${ARMEABI}) --sysroot=${SYSROOT} ${PYTHON_CFLAGS}"
  CPPFLAGS=" ${CFLAGS} -I${COMPILER_ROOT}/include/c++/4.9.x"
#export LDDFLAGS="-fPIC -pie -march=${MARCH} -DANDROID  -grecord-gcc-switches -ffunction-sections -funwind-tables -fstack-protector-strong -no-canonical-prefixes -mstackrealign -Wa,--noexecstack -Wformat -Werror=format-security -std=c11  ${EABISPECIFIC}  ${AFLAGS[${ARMEABI}]} $(getLDflags ${ARMEABI}) --sysroot=${SYSROOT} ${PYTHON_LDFLAGS} ${LDFLAGS}"
 
else

export CFLAGS="-g -O0 -w -Wno-error  ${CFLAGS}"
export CPPFLAGS="-g -O0 -w -Wno-error  ${CPPFLAGS}"
export PREFIX=/usr
...
CONFIG_CMD="../../configure CC='${CC}' CFLAGS='${CFLAGS}' CPPFLAGS='${CPPFLAGS}' --build=${BUILD}  --with-sysroot=${SYSROOT} --host=${HOST} --target=${TARGET} --prefix=${PREFIX}    ${FLAGS}"
echo "///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////"
...
if [ "$OS" = "android" ]; then 
   make DESTDIR=${PWD}/../../dist/${ARMEABI} install   && make DESTDIR=${SYSROOT} install 

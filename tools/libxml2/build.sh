...
export XML2_CONFIG="${SYSROOT}/usr/bin/xml2-config"
 ...
 PYTHON_CFLAGS="-I${COMPILER_ROOT}/include/python2.7"
  PYTHON_LDFLAGS="-L${COMPILER_ROOT}/lib/python2.7"
 

  CFLAGS="${CFLAGS}  -fPIE -fPIC -march=${MARCH} -DANDROID   -ffunction-sections -funwind-tables -fstack-protector-strong -no-canonical-prefixes -mstackrealign -Wa,--noexecstack -Wformat -Werror=format-security -std=c11  ${EABISPECIFIC} ${AFLAGS[${ARMEABI}]}  $(getLDflags ${ARMEABI}) --sysroot=${SYSROOT} ${PYTHON_CFLAGS}"
  CPPFLAGS=" ${CFLAGS} -I${COMPILER_ROOT}/include/c++/4.9.x -latomic"
...
CONFIG_CMD="../../configure CC='${CC}' CFLAGS='${CFLAGS}' CPPFLAGS='${CPPFLAGS}' --build=${BUILD}  --with-sysroot=${SYSROOT} --host=${HOST} --target=${TARGET} --prefix=${PREFIX}  --with-python=${TOOLCHAIN_PATH}  ${FLAGS}"
echo "///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////"
